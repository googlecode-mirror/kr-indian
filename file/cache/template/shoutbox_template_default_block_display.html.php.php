...
<?php else:  if (Phpfox ::getParam('shoutbox.shoutbox_is_live')): ?>
	<script type="text/javascript">
		$Behavior.refreshShouts = function()
		{
			...
			$Core.Shoutbox.sParams = <?php if (isset ( $this->_aVars['aCallbackShoutbox']['module'] )): ?>'module=<?php echo $this->_aVars['aCallbackShoutbox']['module']; ?>&item=<?php echo $this->_aVars['aCallbackShoutbox']['item']; ?>'<?php else: ?>''<?php endif; ?>;
			
			setTimeout("$.ajaxCall('shoutbox.getMessages', $Core.Shoutbox.sParams, 'GET');", <?php echo $this->_aVars['iShoutboxRefresh']; ?>);
		};
	</script>
<?php endif;  endif; ?>
