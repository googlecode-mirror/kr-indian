...
function inputToMap()
{
	...
	if (bDoTrigger)
	{
		google.maps.fevent.trigger(oMarker, 'dragend');
		bDoTrigger = false;
	}
}
...
function initialize()
{
	...
	google.maps.fevent.trigger(oMarker, 'dragstart');
	google.maps.fevent.trigger(oMarker, 'dragend');
	google.maps.fevent.addListener(oMarker, "dragend", function()
	{
		debug('drag end');
		$('#input_gmap_latitude').val(oMarker.position.lat());
		$('#input_gmap_longitude').val(oMarker.position.lng());
	...
	/* Sets fevents for when the user inputs info */
	inputToMap();
}
