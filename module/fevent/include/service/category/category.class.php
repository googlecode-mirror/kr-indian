 ...
 */
class fevent_Service_Category_Category extends Phpfox_Service 
{
	private $_sOutput = '';
	...
	{		
		$sCacheId = $this->cache()->set('fevent_category_browse' . ($iCategoryId === null ? '' : '_' . md5($iCategoryId)));
		...
		{					
			...
			{
				$aCategories[$iKey]['url'] = Phpfox::permalink('fevent.category', $aCategory['category_id'], $aCategory['name']);
				...
				//if ($sCategory === null)
				{
					...
					{
						$aCategories[$iKey]['sub'][$iSubKey]['url'] = Phpfox::permalink('fevent.category', $aSubCategory['category_id'], $aSubCategory['name']);
					}
				}
			}
	...
	{		
		$sCacheId = $this->cache()->set('fevent_parent_breadcrumb_' . md5($sCategory));
	...
	public function getCategoriesById($iId = null, &$aCategories = null)
	{
		...
		{			
			...
			{				
				$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategory['name']), Phpfox::permalink('fevent.category', $aCategory['category_id'], $aCategory['name']));
			}
		}		
		else 
		{			
			$aBreadcrumb[] = array(Phpfox::getLib('locale')->convert($aCategories[0]['name']), Phpfox::permalink('fevent.category', $aCategories[0]['category_id'], $aCategories[0]['name']));
		}
	...
	private function _get($iParentId, $iActive = null)
	{
		...
		if (count($aCategories))
		{
			...
			else 
			{
				...
				$this->_sOutput .= '<option value="">' . ($iParentId === 0 ? Phpfox::getPhrase('fevent.select') : Phpfox::getPhrase('fevent.select_a_sub_category')) . ':</option>' . "\n";
			}
