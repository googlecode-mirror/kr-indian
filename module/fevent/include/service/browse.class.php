	...
	public function query()
	{
		...
		if (Phpfox::isUser() && Phpfox::isModule('like'))
		{
			$this->database()->select('lik.like_id AS is_liked, ')
					->leftJoin(Phpfox::getT('like'), 'lik', 'lik.type_id = \'fevent\' AND lik.item_id = m.fevent_id AND lik.user_id = ' . Phpfox::getUserId());
		}			
	}	
	...
	{		
		...
		foreach ($aNewRows as $aListing)
		{
			...
			$aListing['aFeed'] = array(			
				...
				'feed_link' => Phpfox::getLib('url')->permalink('fevent', $aListing['fevent_id'], $aListing['title']),
