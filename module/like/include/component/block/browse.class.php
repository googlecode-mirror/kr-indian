...
class Like_Component_Block_Browse extends Phpfox_Component
{
	...
	{ 
        ...
        else if ($this->request()->get('type_id') == 'organization')
        {
            $aPage = Phpfox::getService('organization')->getPage($this->request()->getInt('item_id'));
            if (!count($aLikes))
            {
                if ($aPage['type_id'] == 3)
                {
        ...
        $bIsOrganizationAdmin = false;
        if ($this->request()->get('type_id') == 'organization' && Phpfox::getService('organization')->isAdmin($this->request()->getInt('item_id')))
        {
            $bIsOrganizationAdmin = true;
        }
		...
		$this->template()->assign(array(
                ...
                'bIsOrganizationAdmin' => $bIsOrganizationAdmin
			)
		);
	}
