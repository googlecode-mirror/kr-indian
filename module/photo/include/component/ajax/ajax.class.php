        ...
        */ 
        ...
        public function process()
        {
            ...
            if ($iNotCompleted === 0)
            {
                ...
                else if ( ($this->get('fevent_id') > 0) )
                { 
                    $aFevent = Phpfox::getService('fevent')->getForEdit($this->get('fevent_id'));
                    if(!isset($aFevent['fevent_id']))
                    {
                        return false;
                    }
                    $this->call('window.location.href = "' . Phpfox::getLib('url')->permalink('fevent', $this->get('fevent_id'), $aFevent['title']) .'coverupdate_1";');
                }
                else if ($bIsPicup)
                {
